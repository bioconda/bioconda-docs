...
- Open a pull request onto the main ``bioconda-recipes`` repository and see if it passes the tests there (slow).

  - To do that, skip to the [next section](#opening-the-pull-request).

Otherwise, in our Bioconda-build conda environment, we can build via one of two options (in both cases from the root directory of our ``bioconda-recipes`` fork):
